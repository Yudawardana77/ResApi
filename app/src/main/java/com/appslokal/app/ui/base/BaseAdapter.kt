package com.appslokal.app.ui.base
...
import com.appslokal.app.BR

abstract class BaseAdapter <T>(@field:LayoutRes private var layoutid: Int) :
    RecyclerView.Adapter<BaseAdapter<T>.ViewHolder>(), BindableAdapetr<List<T>> {
...
fun <T> recyclerViewData(recyclerView: RecyclerView, data : T?){
    if (recyclerView.adapter is BindableAdapetr<*>){
        (recyclerView.adapter as BindableAdapetr<T>).setData(data)
    }
}
